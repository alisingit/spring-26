...
6. Сравнить качество и время обучения с эталонной реализацией из `scikit-learn`.
...
Датасет загружается во время выполнения скрипта, поэтому в репозитории не хранится отдельный файл с данными.
...
```sh
.venv/bin/python students/grechukha-gv/lab3/source/main.py
```
...
Собственная реализация показывает качество, близкое к эталонной: средний F1 ниже примерно на `0.017`, accuracy ниже примерно на `0.021`. Основной разрыв ожидаем: `sklearn` использует более оптимизированную и зрелую реализацию деревьев и бустинга, а собственная модель намеренно ограничена понятной реализацией CART-деревьев над псевдоостатками.

По времени обучения собственная реализация медленнее примерно в 7 раз. Узкое место — построение большого числа самописных деревьев и перебор порогов в Python/NumPy, тогда как эталонная модель использует оптимизированный код библиотеки.
